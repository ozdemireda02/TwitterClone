...
const [isPicDeleting,setIsPicDeleting] = useState(false);
  ...
  // kaydet butonuna tıklayınca çalışır
  const handleSave = () => {
    // 1.inputtun içeriğine eriş
    const newText = inputRef.current.value;

    // 2.güncellenecek dökumanın referansını al
  ...
  return (
    <>
      ...
      <button
        onClick={close}
        className=" p-2 text-red-400 rounded-full shadow hover:shadow-red-500"
      >
        <ImCancelCircle />
      </button>
      {tweet.imageContent && (
        <div className="relative">
          <img
            className={`${isPicDeleting ? "blur" : ""} my-2 rounded-lg w-full object-contain max-h-[400px]`}
